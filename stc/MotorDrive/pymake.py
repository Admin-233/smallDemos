...
def upload(targetFile, comPort, workingFreq, handshakeBaud = 2400, communicateBaud = 19200):
    command = makeCmd("stcgal", "-t", workingFreq, "--handshake", handshakeBaud, "--baud", communicateBaud, "-p", comPort, '-o reset_pin_enabled=false', targetFile)
    run(command)
...
if cmd == "all":
    ...
    clearDir(OUTPUT_DIR)
    upload(TARGET_FILE, COM, WORKING_FREQ, HANDSHAKE_BAUD, COMMUNICATE_BAUD)
if cmd == "clearall":
